...
class TestConferenceReviewing:
  var cr: ConferenceReviewing = null
  ...
  @Before def init(): Unit =
    ...
    val map: mutable.Map[Question, Int] = mutable.Map()
  ...
  @Test def testOrderedScores(): Unit = // l'articolo 2 ha preso su RELEVANCE i due voti 4,9
  ...
  @Test def testAverageFinalScore(): Unit =  // l'articolo 1 ha preso voto medio su FINAL pari a 8.5, con scarto massimo 0.01
  ...
  @Test def testAcceptedArticles(): Unit =  // solo gli articoli 1,2,4 vanno accettati, avendo media finale >=5 e almeno un voto su RELEVANCE >= 8
    assertEquals(cr.acceptedArticles, Set[Int](1, 2, 4))

  @Test def testSortedAcceptedArticles(): Unit =  // articoli accettati, e loro voto finale medio
    assertEquals(cr.sortedAcceptedArticles, List((4, 7.0), (2, 7.5), (1, 8.5)))

  @Test def optionalTestAverageWeightedFinalScore(): Unit =  // l'articolo 1 ha media pesata finale pari a (4.8+5.4)/2 = 5,1, con scarto massimo 0.01
